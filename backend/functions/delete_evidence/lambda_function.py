"""API Gateway REST API handler for durable S3-first evidence deletion.

IAM: dynamodb:GetItem and dynamodb:DeleteItem on the exact table; s3:DeleteObject
on arn:aws:s3:::<asset-bucket>/evidence/demo/*.
"""
...
REQUIRED_ENVIRONMENT = ("ALLOWED_ORIGIN", "TABLE_NAME", "ASSET_BUCKET")
USER_KEY = "USER#demo"
...
def lambda_handler(event, context):
    """Delete the exact private object before its DynamoDB metadata item."""
    ...
    key = {"PK": USER_KEY, "SK": EVIDENCE_SORT_KEY_PREFIX + evidence_id}
    try: result = get_table(os.environ["TABLE_NAME"]).get_item(Key=key)
    except Exception: return error_response(500, "DEPENDENCY_ERROR", "Evidence could not be deleted.")
    item = result.get("Item")
    if not item: return error_response(404, "NOT_FOUND", "Evidence not found.")
    asset_key = item.get("assetKey")
    if not isinstance(asset_key, str) or not asset_key.startswith(ASSET_KEY_PREFIX): return error_response(500, "DEPENDENCY_ERROR", "Evidence could not be deleted.")
    try: get_s3_client().delete_object(Bucket=os.environ["ASSET_BUCKET"], Key=asset_key)
    except Exception: return error_response(500, "DEPENDENCY_ERROR", "Evidence could not be deleted.")
    try: get_table(os.environ["TABLE_NAME"]).delete_item(Key=key)
    except Exception: return error_response(500, "DEPENDENCY_ERROR", "Evidence could not be deleted.")
    return ApiResponse(204, None, ALLOWED_METHODS).to_dict()
